...
class Public_rfq extends CI_Controller {
    ...
    public function index() {
        ...
        if ($mFilterRfqId || $mFilterRfqStart || $mFilterRfqStatus) {
            ...
            $data['rfqs'] = $this->rfq_model->getAllRFQsByPublicVendorFilter($mUserId, $mFilterRfqId, $mFilterRfqStart, $mFilterRfqStatus);
        } else {
    ...
    public function submit_quote() {
        ...
        if (!empty($items)) {
            ...
            if ($insertid = $this->vendor_model->addQuotedRfq($data)) {
                ...
                $logData = array(
                    'log' => "Quote submitted successfully with the #{$insertid}",
                    'User_Id' => $mVendorId,
                );
