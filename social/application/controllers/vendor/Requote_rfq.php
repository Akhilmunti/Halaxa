...
class Requote_rfq extends CI_Controller {
    ...
    public function index() {
        ...
        if ($mFilterRfqId || $mFilterRfqStart || $mFilterRfqStatus) {
            ...
            $data['quotedRfq'] = $this->vendor_model->getAllQuotedRFQsByVendorAndStatusFilter($mVendorId, $mFilterRfqId, $mFilterRfqStart, $mFilterRfqStatus);
        } else {
    ...
    public function submit_quote() {
        ...
        if (!empty($items)) {
            $data = array(
                ...
                'q_catid' => $ctKey,
                'q_negotiate' => 1,
                'q_negotiate_action' => 1
            );
            if ($insertid = $this->vendor_model->addQuotedRfq($data)) {
                $updateAllRfqs = array(
                    'q_negotiate_action' => 1
                );
                $update = $this->vendor_model->updateQuotedRfqActionRequoteInfo($updateAllRfqs, $rfq_id);
                $file1 = '';
