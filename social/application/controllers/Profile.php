...
class Profile extends CI_Controller {
    ...
    public function getStateByCountry() {
        ...
        $selectedId = $this->input->post('selection_id');
        if ($countryId != '') {
            if ($states = $this->irs_model->getCountryFilter(1, $countryId)) {
                if ($selectedId == $state['location_id']) {
                    $selected = "selected";
                }
                $result = '<option>Select</option>';
                foreach ($states as $key => $state) {
                    $result = $result . "<option value='" . $state['location_id'] . "' " . $selected . ">" . $state['name'] . "</option>" . PHP_EOL;
                }
                echo $result;
            } else {
    ...
    public function getCityByState() {
        ...
        if ($stateId != '') {
            if ($states = $this->irs_model->getCountryFilter(3, $stateId)) {
                $result = '<option>Select</option>';
                foreach ($states as $key => $state) {
                    if ($selectedId == $state['name']) {
                        $selected = "selected";
                    }
                    $result = $result . "<option value='" . $state['name'] . "' " . $selected . ">" . $state['name'] . "</option>" . PHP_EOL;
                }
                echo $result;
            } else {
                echo "<option>No data</option>";
        ...
        if ($typeId != '') {
            if ($courses = $this->irs_model->getCourseFilter($typeId, $parentId)) {
                ...
                foreach ($courses as $key => $course) {
                    if ($selectedId == $course['value']) {
                        $selectedItem = "selected";
                    }
                    $result = $result . "<option value='" . $course['id'] . "' " . $selectedItem . ">" . $course['value'] . "</option>" . PHP_EOL;
                }
                echo $result;
            } else {
                echo "<option>No data</option>";
    ...
    public function getStreamByCourse() {
        ...
        $streamId = $this->input->post('location');
        if ($courseId != '') {
            if ($courses = $this->irs_model->getCourseStreamFilter($courseId)) {
                $result = '<option>Select</option>';
                foreach ($courses as $key => $course) {
                    if ($streamId == $course['specialization']) {
                        $selected = 'selected';
                    }
                    $result = $result . "<option value='" . $course['value'] . "' " . $selected . ">" . $course['value'] . "</option>" . PHP_EOL;
                }
                echo $result;
            } else {
                echo "<option>No data</option>";
    ...
    public function index($id = NULL) {
        ...
        if ($loggedInId) {
            if ($id == $loggedInId) {
                ...
                $userInfo = $this->users_model->get_element_by($id);
                $data['user'] = $userInfo;
                ...
                $data['job_roles'] = $this->irs_model->getJobRoles();
                $mEmail = $userInfo->Email;

                $irs_details = $this->irs_model->getIrsUserByEmail($mEmail);
                ...
                if (!empty($userIrsId)) {
                    ...
                    $data['js_info'] = $this->irs_model->JobSeekerProfileView($userIrsId);
                }
                ...
                //Followers
                $data['connectionsF'] = $this->users_model->getFollowers('Connected', $this->session->userdata('User_Id'));
                ...
                $data['all'] = $associations;
                $mycommunities = $this->association_model->getAllCommunitiesByUser($id);
                $data['myComs'] = $mycommunities;
                $data['navbar'] = "network";
    ...
    public function newProfile($id = NULL) {
        ...
        if (!empty($id)) {
            ...
            $this->load->view('social/profile_new', $data);
        }
    }
    ...
    public function addJsExperienceData($job_seeker_id) {
        if (!empty($job_seeker_id)) {
            $str = rand();
            $resultId = sha1($str);
            $experianceKey = $this->input->post('experiance_key');
            if ($experianceKey) {
                $data = array(
                ...
                $added = $this->irs_model->updateExperienceInfo($data, $experianceKey);
                if ($added == TRUE) {
                    $this->session->set_flashdata('result', 'Experience data updated successfully.');
                } else {
                    $this->session->set_flashdata('result', 'Failed to add data.');
                }
                redirect('profile');
    ...
    public function addJsProjectData($job_seeker_id) {
        if (!empty($job_seeker_id)) {
            $str = rand();
            $resultId = sha1($str);
            $projectKey = $this->input->post('project_key');
            if ($projectKey) {
                $data = array(
                    'job_seeker_id' => $job_seeker_id,
                ...
                $added = $this->irs_model->updateProjectInfo($data, $projectKey);
                if ($added == TRUE) {
                    $this->session->set_flashdata('result', 'Project data updated successfully.');
                } else {
                    $this->session->set_flashdata('result', 'Failed to add data.');
                }
                redirect('profile');
    ...
    public function addJsCertificateData($job_seeker_id) {
        if (!empty($job_seeker_id)) {
            $str = rand();
            $resultId = sha1($str);
            $certificatekey = $this->input->post('certificate_key');
            if ($certificatekey) {
                $data = array(
                    'id' => $resultId,
                    'job_seeker_id' => $job_seeker_id,
                ...
                $added = $this->irs_model->updateCertificateInfo($data, $certificatekey);
                if ($added == TRUE) {
                    $this->session->set_flashdata('result', 'Certificate data updated successfully.');
                } else {
                    $this->session->set_flashdata('result', 'Failed to add data.');
                }
                redirect('profile');
    ...
    public function addJsEduData($job_seeker_id) {
        if (!empty($job_seeker_id)) {
            ...
            $educationKey = $this->input->post('education_key');
            ...
            if ($educationKey) {
                ...
                $added = $this->irs_model->updateEducationInfo($data, $educationKey);
                if ($added == TRUE) {
                    $this->session->set_flashdata('result', 'Educational data Updated successfully.');
                } else {
                    $this->session->set_flashdata('result', 'Failed to add data.');
                }
                redirect('profile');
