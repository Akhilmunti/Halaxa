...
class School_model extends CI_Model {
    ...
    function setSession($data) {
        ...
        $this->session->set_userdata('partner_key', $data['S_key']);
    }
    ...
    public function publishRating($data, $id) {
        $this->db->set($data);
        $this->db->where('id', $id);
    ...
    public function getAllConversationsByPartnerid($id) {
        ...
        $this->db->join('school_info', 'school_info.S_Id = conversations.partner_id');
        $data = array();
    ...
    public function getPubSchoolRatings($id) {
        $this->db->select('*');
        $this->db->where('com_user_id=', $id);
        $this->db->from('ratings');
        //$this->db->join('user_signup', 'user_signup.User_Id = ratings.rated_user_id');
        $data = array();
        $mQuery_Res = $this->db->get();
        if ($mQuery_Res->num_rows() > 0) {
            $data = $mQuery_Res->result_array();
    ...
    public function getSchoolRatings($id) {
        $this->db->select('*');
        $this->db->where('com_user_id=', $id);
        $this->db->from('ratings');
        //$this->db->join('user_signup', 'user_signup.User_Id = ratings.rated_user_id');
    ...
    public function getRatingByComAndUserKey($com, $user) {
        $this->db->select('*');
        $this->db->where('com_user_id=', $com);
        $this->db->where('rated_user_id=', $user);
        $this->db->from('ratings');
        //$this->db->join('user_signup', 'user_signup.User_Id = ratings.rated_user_id');
        $data = array();
        $mQuery_Res = $this->db->get();
    ...
    public function deleteSchoolCommunity($id) {
