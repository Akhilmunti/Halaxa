...
class Users_model extends CI_Model {
    ...
    public function setAdminSession($res) {
        $this->session->set_userdata('admin_username', $res->username);
    ...
    public function getConnectionByConnectedUserId($param) {
        $mUserId = $this->session->userdata('User_Id');
        $this->db->where('Connected_User_Id', $param);
        $this->db->where('User_ID', $mUserId);
        $query = $this->db->get('connection');
        return $query->row_array();
    }

    public function getPartnerInfoById($type, $key) {
        $mUserId = $this->session->userdata('User_Id');
        $this->db->where('partner_type', $type);
        $this->db->where('partner_key', $key);
        $query = $this->db->get('user_signup');
        return $query->row_array();
    }
    ...
    public function getConnectionInfoPartner($to, $logid) {
        $this->db->where('Connected_User_Id', $to);
    ...
    public function getPartnerNotifications($id) {
        $this->db->order_by("id", "desc");
        $this->db->where('to_user', $id);
        $this->db->where('message_type', "Partner");
        $query = $this->db->get('inmail');
        return $query->result_array();
    }
    ...
    public function getAllSentMessages($id) {
        $this->db->where('to_user', $id);
        $query = $this->db->get('inmail');
        return $query->result_array();
    }
    ...
    public function addCatsInfo($data) {
        $query = $this->db->insert('cats_search', $data);
        if ($this->db->affected_rows() > 0) {
            return $this->db->insert_id();
        } else {
            return false;
    ...
    public function addscatsInfo($data) {
        $query = $this->db->insert('scats_search', $data);
        if ($this->db->affected_rows() > 0) {
            return $this->db->insert_id();
        } else {
            return false;
    ...
    public function deleteConnectionByConnectionId($id) {
        $this->db->where('C_ID', $id);
    ...
    public function getFollowers($status, $Id) {
        $Id = $this->session->userdata('User_Id');
        $this->db->select('*');
    ...
    public function getBuyerUserById($id) {
        $this->db->select('*');
        $this->db->where('User_Id =', $id);
        $query = $this->db->get('user_signup');
        if ($query->num_rows() > 0) {
