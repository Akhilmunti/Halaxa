...
class LinkedIn extends AbstractProvider {
    ...
    protected function checkResponse(ResponseInterface $response, $data) {
        ...
        if ($response->getStatusCode() >= 400) {
            ...
            $data['message'] ?: $response->getReasonPhrase(), $data['status'] ?: $response->getStatusCode(), $response
            );
        }
    }
    ...
    protected function checkResponseUnauthorized(ResponseInterface $response, $data) {
        if (isset($data['status']) && $data['status'] === 403) {
            ...
            $data['message'] ?: $response->getReasonPhrase(), $response->getStatusCode(), $response
            );
        }
    }
    ...
    protected function extractEmailFromResponse($response = []) {
        try {
            $confirmedEmails = array_filter($response['elements'], function ($element) {
                return
                        strtoupper($element['type']) === 'EMAIL' && strtoupper($element['state']) === 'CONFIRMED' && $element['primary'] === true && isset($element['handle~']['emailAddress'])
                ;
            });
